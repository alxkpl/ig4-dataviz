...
Pour commencer et utiliser l'interface de `R`-Studio, il faut au préalable télécharger votre propre version de `R` qui n'est pas directement fourni par le logiciel... 
...
- **sous macOS** : allez sur le site du [\color{blue}{CRAN}](https://cran.r-project.org) et cliquez sur le lien "Download R for macOS". Cliquer sur la version la plus récente ("For Apple silicon Macs"). Laissez vous ensuite guider pour l'installation.
...
Pour télécharger `R`-Studio, cliquer sur le lien suivant [\color{blue}{https://posit.co/downloads/}](https://posit.co/downloads/) et télécharger le logiciel via la cellule "DOWNLOAD RSTUDIO" (pas ce lui avec "SERVER").

Suivez les instructions, puis une fois l'installtion terminée, ouvrez le logiciel.
...
Cependant, on aimerait bien rédiger des codes plus complexes sur plusieurs lignes. Pour cela, on a besoin de créer un nouveau script (extension `.R`). Pour ce faire, il suffit d'appuyer sur le petit carré blanc $+$ vert, situé en haut  gauche de l'interface. Cliquer ensuite sur "R script" et normanelent un cript `R` vierge s'est ouvert comme ceci :  
\newpage
...
On rappelle que l'on peut déclarer une varaible de la manière suivante : 
...
Bien sur, il est nécessaire de compiler cette commande afin d'enregistrer les modifications. Pour compiler une ligne dans un script, il faut d'abord placer le curseur sur la ligne correspondante puis on appuie sur le bouton "Run" situé en haut à droite du script. (Si vous voulez compiler plusieurs lignes d'un coup, il faut sélectionner les lignes correspondantes puis appuyer sur "Run").

> **Exercice.** Créer une variable $n$ prenant pour valeur 10. Que se passe-t-il dans l'onglet "Environment" ?
...
*Bien sur, il y a une correspondance numérique à chacune de ces valeurs. `FALSE` correspond à la valeur 0 et `TRUE` à la valeur 1. Ainsi, on peut sommer deux booléens pour obtenir une valeur numérique (cette valeur indique dans combien de fois l'expression est vraie).*
...
> **Exercice** En utilisant des inégalités et des opérateurs logiques, répondez à la question : est ce que le chiffre $\pi$ (enregistrer par défaut sur `R` par la variable `pi`) est compris entre $3,14159265$ et $3,14159266$ ?
...
*Bien sur, on peut avoir accès au $i$-ème élément d'un vecteur en écrivant `vecteur[i]`. Les opérations "classiques" (que ce soit numérique ou booléen) pour les nombres sont utilisables entre deux vecteurs.*
...
3. En utilisant la fonction `sum()` (tester la fonction avant pour voir ce qu'elle fait), comptez le nombre d'élements de la liste `vect2` qui sont plus grands que 3. De même, comptez le nombre d'élements de la liste `vect2` qui sont plus grands que `vect1` (terme à terme).


Il y a aussi pleins de petites fonctions utiles qui peuvent servir pour manipuler des vecteurs. 

>
**Exercice.** Appliquez les fonctions `max`, `min`, `which.max`, `which.min`,`sort` sur `vect1` et commenter leur sortie.
...
- cliquez sur l'onglet "Packages" dans l'interface, puis sur "Install" puis recherchez le nom du package que vous voulez installer puis ré-appuyez sur "Install".
...
Pour ce faire, cliquez qur le petit cube avec un "R" à l'ntérieur (situé à coté du bouton pour créer les scripts). Cliquez sur "New Directory" puis sur "New project". Ensuite donner un nom à votre projet `R` (par exemple DO3-DataViz) et sélectionner le dossier dans lequel vous voulez le placer (en général je crée un nouveau dossier avec le nom du projet).
...
Avoir ouvert `R`-Studio via un projet va également vous permettre d'utilser d'autres librairies utiles comme par exemple `here`.

> **Exercice.** Installez la librairie `here`. Après l'avoir importer, qu'affiche la sortie de `here()` ? Et celle de `here("src")` ? Et `here("src", "TP1.R")` ?
...
lancers <- sample(c("Pile", "Face"),    # les issues possibles
                  ...
                  replace = TRUE,       # permet d'avoir plusieurs face ou pile
                  prob = c(0.5, 0.5))   # proba de tombé sur pile ou face 
print(lancers)
```

> **Question 1.** Copiez et compilez ce code plusieurs fois, que remarquez-vous ? Comme il s'agit d'une simulation, il est parfois utile d'utiliser une graine (*seed* en anglais) pour la reporductibilité des résultats. Au dessus de votre copier-coller, écrivez `set.seed(2025)` et réexécuter à nouveau plusieurs fois le code. Y-a-t'il un changement ? Et si vous remplacez $2025$ par une autre valeur ?

Bien sûr, il s'agit juste d'un exemple jouet. Il est parfois plus compliqué de simuler des données surtout lorsqu'elles sont continues. Heureusement pour nous, `R` est fait pour les statisticien.nes, et il existe des fonctions pour simuler sous des distributions plus complexes (par exemple `runif()` va simuler des réalisations de loi uniforme sur l'intervalle $[0,1]$).
